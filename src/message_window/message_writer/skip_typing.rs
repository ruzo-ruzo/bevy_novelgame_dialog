...
pub fn skip_or_next (
    ...
    mut typing_texts_query: Query<(Entity, &mut TypingStyle, &mut Transform, &Parent), With<MessageTextChar>>,
            ...
            if let Ok((tb_tf, tb_sp)) = text_box_query.get(tb_entity) {
                if text_count == typed_count {
                    if let Ok(ref_value) = read_ron(&type_registry, ron.clone()) {
                        commands.add(|w: &mut World| { w.send_event(BMSEvent{ value:  ref_value}); } )
                    }
                } else {
...
pub fn make_wig_for_textbox(
    ...
    let ron_iff_opt = write_ron(type_registry, InputForFeeding { target_text_box: Some(tb_entity) });
    let ron_ifs_opt = write_ron(type_registry, InputForSkipping {
