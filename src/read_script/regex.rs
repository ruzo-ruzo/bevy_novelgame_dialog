...
pub fn replace_by_template<S1: AsRef<str>, S2: AsRef<str>>(input: S1, template: S2) -> String {
    ...
    template_vec.into_iter().fold(
        input.as_ref().to_string(),
        |base, (from, to)| Regex::new(&from).unwrap().replace_all(&base, to).to_string()
    )
}
...
fn quoted_cell(s: &str) -> impl FnMut(&str) -> IResult<&str, (String, bool)>  + '_ {
    move |input| {
        let empty = pair(value("".to_string(), tuple((space0, tag(s), tag(s)))), cell_ending);
        let double_quote_open = tuple((space0, tag(s), not(tag(s))));
        let escaped_double_quote = value(s.to_string(), preceded(tag(s), tag(s)));
        let non_closing = map(not(quote_close(s)).and(take(1usize)), |(_, x)| x.to_string());
...
fn non_quoted_cell(input: &str) -> IResult<&str, (String, bool)> {
    map(many_till(take(1usize), cell_ending), |(v, b)|(v.concat(), b))(input)
}

fn cell_ending(input: &str) -> IResult<&str, bool> {
    preceded(space0, alt((value(false, tag(",")), value(true, line_ending))))(input)
}
...
mod parse_template_tests {
   ...
   fn str_refs2strings(base: Vec<Vec<&str>>) -> Vec<Vec<String>> {
       base.iter().map(|x|x.iter().map(|y|y.to_string()).collect()).collect()
   }
